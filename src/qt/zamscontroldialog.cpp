// Copyright (c) 2017 The CreaCoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zamscontroldialog.h"
#include "ui_zamscontroldialog.h"
...
std::list<std::string> ZAMSControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAMSControlDialog::listMints;

ZAMSControlDialog::ZAMSControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAMSControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAMSControlDialog::~ZAMSControlDialog()
{
    delete ui;
}

void ZAMSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAMSControlDialog::updateList()
{
...
void ZAMSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAMSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAMS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAMSControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAMSControlDialog::GetSelectedMints()
{
...
void ZAMSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
