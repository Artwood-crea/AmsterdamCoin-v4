...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CreaCoin address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no CreaCoin: URI
...
{
    // Convert creacoin:// to creacoin:
    //
    //    Cannot handle this later, because creacoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open creacoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CreaCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CreaCoin.lnk
...
{
    return GetAutostartDir() / "creacoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a creacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CreaCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the creacoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add creacoin app to startup item list
