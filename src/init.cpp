...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CREAC and zCREAC money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CREAC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CREAC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, amsterdamcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CREAC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AmsterdamCoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzams=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCREAC minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeamsterdamcoinamount=<n>     " + strprintf(_("Keep N CREAC anonymized (default: %u)"), 0) + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // ignore SIGPIPE on AmsterdamCoin
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // AmsterdamCoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCREACMinted();
                        RecalculateZCREACSpent();
                    }
                    RecalculateCREACSupply(1);
                }
                ...
                // AmsterdamCoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZCREACBackups = GetBoolArg("-backupzams", true);
        pwalletMain->setZCREACAutoBackups(fEnableZCREACBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeAmsterdamCoinAmount < 2) nAnonymizeAmsterdamCoinAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1CREAC+1000 == (.1CREAC+100)*10
       10CREAC+10000 == (1CREAC+1000)*10
    */
